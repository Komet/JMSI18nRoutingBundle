...
class DefaultPatternGenerationStrategy implements PatternGenerationStrategyInterface
{
    ...
    public function __construct($strategy, TranslatorInterface $translator, LocaleProviderInterface $localeProvider, $cacheDir, $translationDomain = 'routes')
    {
    ...
    {
        $patterns = array();
        foreach ($route->getOption('i18n_locales') ?: $this->localeProvider->getLocales() as $locale) {
            ...
            if (self::STRATEGY_PREFIX === $this->strategy
                || (self::STRATEGY_PREFIX_EXCEPT_DEFAULT === $this->strategy && $this->localeProvider->getDefaultLocale() !== $locale)) {
